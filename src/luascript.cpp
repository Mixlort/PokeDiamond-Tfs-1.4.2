...
void LuaScriptInterface::registerFunctions()
{
    ...
    registerMethod("Creature", "getCreatureIcon", LuaScriptInterface::luaGetCreatureIcon);
    registerMethod("Creature", "setCreatureIcon", LuaScriptInterface::luaSetCreatureIcon);
    ...
    registerMethod("Creature", "getStorageStringValue", LuaScriptInterface::luaCreatureGetStorageStringValue);
    registerMethod("Creature", "setStorageStringValue", LuaScriptInterface::luaCreatureSetStorageStringValue);   
    ...
    //mixlort
    // registerMethod("MonsterType", "levelToUse", LuaScriptInterface::luaMonsterTypelevelToUse);
    ...
    registerMethod("MonsterType", "hungerDelay", LuaScriptInterface::luaMonsterTypeHungerDelay);
...
int LuaScriptInterface::luaDoAreaCombat(lua_State* L)
{
	...
	if (area || areaId == 0) {
		...
		CombatDamage damage;
		damage.origin = getNumber<CombatOrigin>(L, 9, ORIGIN_SPELL);
...
int LuaScriptInterface::luaGameCreateMonster(lua_State* L)
{
    ...
    Monster* monster = Monster::createMonster(getString(L, 1), bst);
	if (!monster) {
		lua_pushnil(L);
		return 1;
	}
...
int LuaScriptInterface::luaSetCreatureIcon(lua_State* L)
{
    //setCreatureIcon(cid, icon)
    uint8_t icon = getNumber<uint8_t>(L, 2);
...
int LuaScriptInterface::luaGetCreatureIcon(lua_State* L)
{
    //getCreatureIcon(cid)
    Creature* creature = getCreature(L, 1);
    if (!creature) {
...
int LuaScriptInterface::luaCreatureGetStorageStringValue(lua_State* L)
{
    // creature:getStorageStringValue(key)
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
        return 1;
    ...
    uint32_t key = getNumber<uint32_t>(L, 2);
	std::string strValue;
    if (creature->getStorageStringValue(key, strValue)) {
...
int LuaScriptInterface::luaCreatureSetStorageStringValue(lua_State* L)
{
    // creature:setStorageStringValue(key, value)
    Creature* creature = getUserdata<Creature>(L, 1);
    if (!creature) {
        lua_pushnil(L);
        return 1;
    ...
    if (lua_isnoneornil(L, 3)) {
        ...
        creature->addStorageStringValue(key, getString(L, 3));
	}
...
int32_t LuaScriptInterface::luaCreatureMoveTo(lua_State* L)
{
	...
	if (creature->getMaster()) {
		creature->getMaster()->getCreature()->setOrder("walk", true);
		creature->setFollowCreature(nullptr);
	}

	std::forward_list<Direction> dirList;
	std::vector<Direction> convertedDirList(dirList.begin(), dirList.end());
	if (creature->getPathTo(position, convertedDirList, fpp)) {
		creature->hasFollowPath = true;
		creature->startAutoWalk(convertedDirList);
		pushBoolean(L, true);
	}
...
// int LuaScriptInterface::luaMonsterTypelevelToUse(lua_State* L)
// {
//     // monsterType:levelToUse()
//     MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
//     if (monsterType) {
//         lua_pushnumber(L, monsterType->info.levelToUse);
//     } else {
//         lua_pushnil(L);
//     }
//     return 1;
...
int LuaScriptInterface::luaMonsterTypeHungerDelay(lua_State* L)
{
	// monsterType:getHungerDelay() 
	MonsterType* monsterType = getUserdata<MonsterType>(L, 1);
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			lua_pushnumber(L, monsterType->info.hungerDelay);
		} else {
			monsterType->info.hungerDelay = getNumber<uint32_t>(L, 2);
			pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaMonsterTypeRace2(lua_State* L) //pota
{
