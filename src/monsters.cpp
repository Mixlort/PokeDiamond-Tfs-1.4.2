...
bool Monsters::deserializeSpell(const pugi::xml_node& node, spellBlock_t& sb, const std::string& description)
{
    ...
    if ((attr = node.attribute("level"))) {//pota
        sb.level = pugi::cast<int32_t>(attr.value());
    }    

    if ((attr = node.attribute("f"))) {//pota
        sb.f = pugi::cast<int32_t>(attr.value());
    }    

    if ((attr = node.attribute("t"))) {//pota
        sb.t = attr.as_string();
    }        

    if ((attr = node.attribute("passive"))) {//pota
        sb.passive = attr.as_string();
    }        
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			if (minSpeedChange >= 0) {
				...
				combat->setParam(COMBAT_PARAM_AGGRESSIVE, 0);
            } else if (minSpeedChange == -100) { //pota
...
MonsterType* Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading /*= false*/)
{
	...
	if ((attr = monsterNode.attribute("race"))) {
        ...
        } else if (tmpStrValue == "no race" || tmpInt == 24) {
            mType->info.race = RACE_NONE;
		} else {
	...
	if ((node = monsterNode.child("flags"))) {
		for (auto flagNode : node.children()) {
			...
			} else if (strcasecmp(attrName, "hungerdelay") == 0) {
				mType->info.hungerDelay = pugi::cast<int32_t>(attr.value());
	...
	if ((node = monsterNode.child("elements"))) {
		for (auto elementNode : node.children()) {
			if ((attr = elementNode.attribute("physicalPercent"))) {
                ...
                if (mType->info.damageImmunities & COMBAT_PSYCHICDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"psychic\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_GRASSDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"grass\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_NORMALDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"normal\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_FLYINGDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"flying\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_ELECTRICDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"electric\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_GROUNDDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"ground\" on immunity and element tags. " << file << std::endl;
                }
            } else if ((attr = elementNode.attribute("rockPercent"))) { //pota
                mType->info.elementMap[COMBAT_ROCKDAMAGE] = pugi::cast<int32_t>(attr.value());
                if (mType->info.damageImmunities & COMBAT_ROCKDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"rock\" on immunity and element tags. " << file << std::endl;
                }
            } else if ((attr = elementNode.attribute("bugPercent"))) { //pota
                mType->info.elementMap[COMBAT_BUGDAMAGE] = pugi::cast<int32_t>(attr.value());
                if (mType->info.damageImmunities & COMBAT_BUGDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"bug\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_DRAGONDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"dragon\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_GHOSTDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"ghost\" on immunity and element tags. " << file << std::endl;
                }
            } else if ((attr = elementNode.attribute("darkPercent"))) { //pota
                mType->info.elementMap[COMBAT_DARKDAMAGE] = pugi::cast<int32_t>(attr.value());
                if (mType->info.damageImmunities & COMBAT_DARKDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"dark\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_STEELDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"steel\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_FAIRYDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"fairy\" on immunity and element tags. " << file << std::endl;
                }
                ...
                if (mType->info.damageImmunities & COMBAT_FIGHTINGDAMAGE) {
                    std::cout << "[Warning - Monsters::loadMonster] Same element \"fighting\" on immunity and element tags. " << file << std::endl;
                }
