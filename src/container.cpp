		...
		if (index == INDEX_WHEREEVER) {
			...
			for (Item* containerItem : itemlist) {
				if (containerItem != item && containerItem->equals(item) && containerItem->getItemCount() < item->getItemMaxCount()) {
					if (queryAdd(slotIndex++, *item, count, flags) == RETURNVALUE_NOERROR) {
						n += item->getItemMaxCount() - containerItem->getItemCount();
					}
				}
			}
		} else {
			...
			if (item->equals(destItem) && destItem->getItemCount() < item->getItemMaxCount()) {
				if (queryAdd(index, *item, count, flags) == RETURNVALUE_NOERROR) {
					n = item->getItemMaxCount() - destItem->getItemCount();
				}
			}
		}
		...
		uint32_t& flags)
{
	...
	if (autoStack && item->isStackable() && item->getParent() != this) {
		if (*destItem && (*destItem)->equals(item) && (*destItem)->getItemCount() < item->getItemMaxCount()) {
			return this;
		}
