...
ReturnValue Combat::canTargetCreature(Player* attacker, Creature* target)
{
    ...
    if (attacker->getPlayer() && target->isSummon() && target->getMaster() && (target->getMaster()->getPlayer() == attacker->getPlayer())) { // player não atacar o próprio summon
        return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
    }

    if (attacker->getPlayer() && target->getPlayer() && (target->getPlayer()->getSummonCount() >= 1)) {
        return RETURNVALUE_YOUMAYNOTATTACKTHISPLAYER;
    }
    ...
    if (target->isSummon() && target->getMaster() && target->getMaster()->getPlayer()) {
        if (isProtected(attacker, target->getMaster()->getPlayer())) {
            return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
        }
    }
	...
	if (const Player* targetPlayer = target->getPlayer()) {
		...
		if (const Player* attackerPlayer = attacker->getPlayer()) {
            ...
            if (target->isSummon() && target->getMaster() == attacker) {
                return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
            } // player não atacar seu summon // mixlort ativar
            ...
            if (targetMaster && targetMaster->getPlayer() && attacker->getMaster() && attacker->getMaster()->getPlayer() && isProtected(attacker->getMaster()->getPlayer(), targetMaster->getPlayer())) {
                return RETURNVALUE_YOUMAYNOTATTACKTHISCREATURE;
            } // protectlevel mixlort
