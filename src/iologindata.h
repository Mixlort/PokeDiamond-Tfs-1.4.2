...
using PokemonInfo = std::pair<uint16_t, std::string>;
using PokemonInfoList = std::list<PokemonInfo>;
...
class IOLoginData
{
	public:
        ...
        static PokemonInfoList getPlayerPokemonTeam(const std::string& name);
        static uint16_t getPlayerLevel(const std::string& name);
        static uint8_t getPlayerVocation(const std::string& name);
        static Outfit_t getPlayerOutfit(const std::string& name);
