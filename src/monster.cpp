...
Monster* Monster::createMonster(const std::string& name, uint16_t bst) //pota
{
	MonsterType* mType = g_monsters.getMonsterType(name);
	if (!mType) {
		return nullptr;
	}
    return new Monster(mType, bst);
}

Monster::Monster(MonsterType* mType, uint16_t bst) : //pota
	Creature(),
...
bool Monster::selectTarget(Creature* creature)
{
    ...
    Creature* target = creature->isPlayerSummon() ? creature->getMaster() : creature;
    if(isPassive() && !hasBeenAttacked(target->getID()) && target->getAttackedCreature() != this) {
        return false;
    }
  
    if(Creature* summonOne = creature->pushBackSummonOne()) {
        if(setAttackedCreature(summonOne)) {
            g_dispatcher.addTask(createTask(std::bind(&Game::checkCreatureAttack, &g_game, summonOne->getID())));
        }
        return setFollowCreature(summonOne);
    }
            ...
            if (isSummon()) {
                if (!attackedCreature) {
                    if (getMaster() && getMaster()->getAttackedCreature()) {
                        ...
                        if (getMaster()) {
                            getMaster()->getCreature()->setOrder("walk", false);
                        }
                    } else if (getMaster() != followCreature && getMaster()->getCreature()->isOrder("walk") != true) {
...
void Monster::doAttacking(uint32_t interval)
{
    ...
    if(attackedCreature->getPlayer()) {
       if(Creature* summon = attackedCreature->pushBackSummonOne()) {
           if(this->getPathTo(summon->getPosition(), dirList, fpp)){
              selectTarget(summon);
           }
        } 
    }

    // Tile* toTile = g_game.map.getTile(targetPos);
    // if (toTile && toTile->hasFlag(TILESTATE_PROTECTIONZONE)) {
    //  return;
    // } // ARRUMAR
...
void Monster::onThinkTarget(uint32_t interval)
{
	if (!isSummon()) {
		...
		// 		}
		// 	} else if (Monster* monster = attackedCreature->getMonster()) {
		// 		if (monster->belongsToPlayer() && monster->getMaster()->getZone() == ZONE_PROTECTION) {
...
void Monster::onThinkDefense(uint32_t interval)
{
	...
	if (!isSummon() && summons.size() < mType->info.maxSummons && hasFollowPath) {
		for (const summonBlock_t& summonBlock : mType->info.summons) {
            ...
            Monster* summon = Monster::createMonster(summonBlock.name, 0); //pota
			if (summon) {
