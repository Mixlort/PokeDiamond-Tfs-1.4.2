...
void House::updateDoorDescription() const
{
	...
	for (const auto& it : doorSet) {
		it->setSpecialDescription(fmt::format("It belongs to house '{:s}'. {:s} owns this house.{:s}", houseName, (owner != 0) ? ownerName : "Nobody", g_config.getBoolean(ConfigManager::HOUSE_DOOR_SHOW_PRICE) && (housePrice != -1) && (owner == 0) ? fmt::format(" It costs {:d} dollars.", ((houseTiles.size() * housePrice) / 100.0f)) : ""));
	}
}
	...
	for (const auto& it : houseMap) {
		...
		if (player.getBankBalance() >= rent) {
			...
			if (house->getPayRentWarnings() < 7) {
				...
				letter->setText(fmt::format("Warning! \nThe {:s} rent of {:d} dollars for your house \"{:s}\" is payable. Have it within {:d} days or you will lose this house.", period, (house->getRent() / 100.0f), house->getName(), daysLeft));
