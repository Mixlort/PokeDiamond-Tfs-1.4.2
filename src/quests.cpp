...
{
    auto storage = player->getStorageValue(storageID);

	if (!mainDescription.empty()) {
		std::string desc = mainDescription;
        replaceString(desc, "|STATE|", std::to_string(storage.value_or(0)));
		replaceString(desc, "\\n", "\n");
		return desc;
	}
	...
	return "Digite !task para ver seu progresso na missão atual.";
}
...
bool Mission::isStarted(Player* player) const
{
    ...
    if (storage.value_or(0) < startValue) {
        return false;
    }

    if (!ignoreEndValue && storage.value_or(0) > endValue) {
        return false;
    }
...
bool Mission::isCompleted(Player* player) const
{
    ...
    if (ignoreEndValue) {
        return storage.value_or(0) >= endValue;
    }
...
bool Quest::isStarted(Player* player) const
{
    ...
    auto storage = player->getStorageValue(startStorageID);
    if (storage.value_or(0) < startStorageValue) {
        return false;
    }
