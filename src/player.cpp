...
void Player::onWalk(Direction& dir)
{
	...
	// setNextAction(OTSYS_TIME() + getStepDuration(dir)); // mixlort delay ao usar item e andar
}
...
void Player::addExperience(Creature* source, uint64_t exp, bool sendText/* = false*/)
{
	...
	while (experience >= nextLevelExp) {
		...
		// manaMax += vocation->getManaGain(); // ganhar mana quando upar
		// mana += vocation->getManaGain(); // ganhar mana quando upar
...
bool Player::tameShader(uint8_t shaderId)
{
    ...
    setStorageValue(key, getStorageValue(key).value_or(0) | (1 << (tmpShaderId % 31)));
    return true;
}

bool Player::untameShader(uint8_t shaderId)
{
    if (!g_game.shaders.getShaderByID(shaderId)) {
        return false;
    }

    uint8_t tmpShaderId = shaderId - 1;
    uint32_t key = PSTRG_SHADERS_RANGE_START + (tmpShaderId / 31);
...
bool Player::tameAura(uint8_t auraId)
{
    ...
    setStorageValue(key, getStorageValue(key).value_or(0) | (1 << (tmpAuraId % 31)));
    return true;
}
...
bool Player::untameAura(uint8_t auraId)
{
    ...
    uint8_t tmpAuraId = auraId - 1;
    uint32_t key = PSTRG_AURAS_RANGE_START + (tmpAuraId / 31);
...
bool Player::tameWing(uint8_t wingId)
{
    ...
    setStorageValue(key, getStorageValue(key).value_or(0) | (1 << (tmpWingId % 31)));
    return true;
}
...
bool Player::untameWing(uint8_t wingId)
{
    ...
    uint8_t tmpWingId = wingId - 1;
    uint32_t key = PSTRG_WINGS_RANGE_START + (tmpWingId / 31);
...
bool Player::hasWing(const Wing* wing) const
{
    ...
    uint8_t tmpWingId = wing->id - 1;
    uint32_t key = PSTRG_WINGS_RANGE_START + (tmpWingId / 31);

    return ((1 << (tmpWingId % 31)) & getStorageValue(key).value_or(0)) != 0;
}

uint8_t Player::getCurrentWing() const
{
    auto storage = getStorageValue(PSTRG_WINGS_CURRENTWINGS);
    return storage.value_or(0);
}

void Player::setCurrentWing(uint8_t wingId)
{
    setStorageValue(PSTRG_WINGS_CURRENTWINGS, wingId);
}
...
bool Player::hasAura(const Aura* aura) const
{
    ...
    uint8_t tmpAuraId = aura->id - 1;
    uint32_t key = PSTRG_AURAS_RANGE_START + (tmpAuraId / 31);

    return ((1 << (tmpAuraId % 31)) & getStorageValue(key).value_or(0)) != 0;
}

uint8_t Player::getCurrentAura() const
{
    auto storage = getStorageValue(PSTRG_AURAS_CURRENTAURA);
    return storage.value_or(0);
}

void Player::setCurrentAura(uint8_t auraId)
{
    setStorageValue(PSTRG_AURAS_CURRENTAURA, auraId);
}
...
bool Player::hasShader(const Shader* shader) const
{
    ...
    uint8_t tmpShaderId = shader->id - 1;
    uint32_t key = PSTRG_SHADERS_RANGE_START + (tmpShaderId / 31);

    return ((1 << (tmpShaderId % 31)) & getStorageValue(key).value_or(0)) != 0;
}
