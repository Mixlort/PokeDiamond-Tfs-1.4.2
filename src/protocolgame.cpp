...
void ProtocolGame::sendCreatureSkull(const Creature* creature)
{
    ...
    {
        if(monster->isSummon() || !monster->isSummon())
        {
            boost = monster->getBoost();
        }
    }
...
void ProtocolGame::sendContainer(uint8_t cid, const Container* container, bool hasParent, uint16_t firstIndex)
{
	...
	if (firstIndex < containerSize) {
		...
		for (auto it = container->getItemList().begin() + firstIndex, end = it + itemsToSend; it != end; ++it) {
            ...
            Item* item = *it;
            const ItemAttributes::CustomAttribute* customAttributeName = item->getCustomAttribute("pokeName");
            if (customAttributeName) {
                const std::string* namePokePtr = boost::get<std::string>(&customAttributeName->value);
                if (namePokePtr) {
                    std::string pname = *namePokePtr;
                    msg.addString(pname);
                    const ItemAttributes::CustomAttribute* customAttributeStatus = item->getCustomAttribute("statusBall");
                    if (customAttributeStatus) {
                        const int64_t* statusBallPtr = boost::get<int64_t>(&customAttributeStatus->value);
                        if (statusBallPtr) {
                            uint8_t statusBall = static_cast<uint8_t>(*statusBallPtr);
                            msg.addByte(static_cast<unsigned int>(statusBall));
                        } 
                    } else {
                        msg.addByte(0);
                    }
...
void ProtocolGame::sendInventoryItem(slots_t slot, const Item* item)
{
	...
	if (item) {
        ...
        Item* itemAtt = const_cast<Item*>(item);
        const ItemAttributes::CustomAttribute* customAttributeName = itemAtt->getCustomAttribute("pokeName");
        if (customAttributeName) {
            const std::string* namePokePtr = boost::get<std::string>(&customAttributeName->value);
            if (namePokePtr) {
                std::string pname = *namePokePtr;
                msg.addString(pname);
                const ItemAttributes::CustomAttribute* customAttributeStatus = itemAtt->getCustomAttribute("statusBall");
                if (customAttributeStatus) {
                    const int64_t* statusBallPtr = boost::get<int64_t>(&customAttributeStatus->value);
                    if (statusBallPtr) {
                        uint8_t statusBall = static_cast<uint8_t>(*statusBallPtr);
...
void ProtocolGame::AddCreature(NetworkMessage& msg, const Creature* creature, bool known, uint32_t remove)
{
    ...
    {
        if(monster->getBoost()){
          b = std::to_string(monster->getBoost());
        }
    }
