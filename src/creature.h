...
static constexpr int32_t EVENT_CREATURE_THINK_INTERVAL = 500; // Mixlort se o server ficar muito pesado aumentar pra 1000
...
class Creature : virtual public Thing
{
	...
	public:
        ...
        Player* getPlayerMaster() const {
            return isPlayerSummon() ? master->getPlayer() : nullptr;
        }
		bool isSummon() const {
		...
		void addStorageStringValue(const uint32_t key, const std::string& value);
		bool getStorageStringValue(const uint32_t key, std::string& value) const;
        decltype(auto) getStorageStringMap() const { return storageStringMap; }

        bool isOrder(std::string stat) {
            return Order[stat] != 0;
        }
        void setOrder(std::string stat, bool value) {
            Order[stat] = value ? 1 : 0;
        }
	...
	protected:
		...
		uint32_t baseSpeed = 440; //mixlort 220
		int32_t varSpeed = 0;
		int32_t health = 1000;
