...
void ProtocolLogin::getCharacterList(const std::string& accountName, const std::string& password, const std::string& token, uint16_t version)
{
	...
	for (uint8_t i = 0; i < size; i++) {
        ...
        //outfit window
        output->add<uint16_t>(IOLoginData::getPlayerLevel(character));
        output->addByte(IOLoginData::getPlayerVocation(character));
        Outfit_t outfit = IOLoginData::getPlayerOutfit(character);
        PokemonInfoList pokemonInfoList = IOLoginData::getPlayerPokemonTeam(character);

        output->add<uint16_t>(outfit.lookType);
        output->add<uint16_t>(outfit.lookHead);
        output->add<uint16_t>(outfit.lookBody);
        output->add<uint16_t>(outfit.lookLegs);
        output->add<uint16_t>(outfit.lookFeet);
        output->add<uint16_t>(outfit.lookAddons);
        ...
        for(PokemonInfoList::const_iterator pInfoIt = pokemonInfoList.begin(); pInfoIt != pokemonInfoList.end(); ++pInfoIt) {
            output->add<uint16_t>(pInfoIt->first);
