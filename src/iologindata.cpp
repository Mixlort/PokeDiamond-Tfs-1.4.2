...
bool IOLoginData::loadPlayer(Player* player, DBResult_ptr result)
{
	...
	if ((result = db.storeQuery(fmt::format("SELECT `key`, `value` FROM `player_storagestring` WHERE `player_id` = {:d}", player->getGUID())))) {
		do {
			player->addStorageStringValue(result->getNumber<uint32_t>("key"), result->getString("value"));
		} while (result->next());
	}
...
bool IOLoginData::saveItems(const Player* player, const ItemBlockList& itemList, DBInsert& query_insert, PropWriteStream& propWriteStream, std::map<Container*, int>& openContainers)
{
	...
	for (const auto& it : itemList) {
        ...
        if (Container* container = item->getContainer()) {
            auto itCont = openContainers.find(container);
            if (itCont == openContainers.end()) {
                container->resetAutoOpen();
            } else {
                container->setAutoOpen(itCont->second);
            }
...
bool IOLoginData::savePlayer(Player* player)
{
	...
	if (player->lastDepotId != -1) {
		...
		for (const auto& itDepot : player->depotChests) {
			for (Item* item : itDepot.second->getItemList()) {
				itemList.emplace_back(itDepot.first, item);
			}
		}
    ...
    //save storage string map
	if (!db.executeQuery(fmt::format("DELETE FROM `player_storagestring` WHERE `player_id` = {:d}", player->getGUID()))) {
		return false;
	}

    DBInsert storagestringQuery("INSERT INTO `player_storagestring` (`player_id`, `key`, `value`) VALUES ");
    for (const auto& [key, value] : player->getStorageStringMap()) {
        if (!storagestringQuery.addRow(fmt::format("{:d}, {:d}, {}", player->getGUID(), key, db.escapeString(value)))) {
            return false;
        }
    }
...
{    
    ...
    uint32_t guid = getGuidByName(name);;
    DBResult_ptr result = Database::getInstance().storeQuery(fmt::format("SELECT `slot`, `pokemon_number`, `description` FROM `player_pokemon` WHERE `player_id` = {:d}", guid));
    if (result) {
        do {
            pokemonInfoList.push_back(PokemonInfo(result->getNumber<uint32_t>("pokemon_number"), result->getString("description")));
        } while (result->next());
    }
...
uint16_t IOLoginData::getPlayerLevel(const std::string& name)
{
    Database& db = Database::getInstance();

    DBResult_ptr result = db.storeQuery(fmt::format("SELECT `level` FROM `players` WHERE `name` = {:s}", db.escapeString(name)));
    if (!result) {
        return 0;
    }
    return result->getNumber<uint16_t>("level");
}
...
uint8_t IOLoginData::getPlayerVocation(const std::string& name)
{
    ...
    DBResult_ptr result = db.storeQuery(fmt::format("SELECT `vocation` FROM `players` WHERE `name` = {:s}", db.escapeString(name)));
    if (!result) {
        return 0;
    }
    return result->getNumber<uint32_t>("vocation");
}
...
Outfit_t IOLoginData::getPlayerOutfit(const std::string& name)
{
    ...
    Database& db = Database::getInstance();
    DBResult_ptr result = db.storeQuery(fmt::format("SELECT `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype`, `lookaddons` FROM `players` WHERE `name` = {:s}", db.escapeString(name)));
    if (!result) {
        return outfit;
    }
