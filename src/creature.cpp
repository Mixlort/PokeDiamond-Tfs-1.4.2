	...
	if (creature == this) {
		...
		if (!summons.empty()) {
			...
			for (Creature* summon : summons) {
                ...
                if (Position::getDistanceZ(newPos, pos) > 0 || (Position::getDistanceX(newPos, pos) > 8) || (Position::getDistanceY(newPos, pos) > 6)) {
                    //changed, before: despawnList.push_front(summon);
                    g_game.map.moveCreature(*summon, *destTile);                    
                    //added
                    g_game.addMagicEffect(pos, CONST_ME_TELEPORT); // Mixlort Teleport Poke
				}
			}
...
//     bool ret = false;
//     auto storage = this->getStorageValue(5032);
//     if(storage.value_or(0) > 0)
//     {
//         ret = true;
//     }
//     return ret;
...
{
	if (creature) {
        // if(creature->getPlayer()) { // mixlort proteção contra attack player com summon
        //     if(Creature* summon = creature->pushBackSummonOne()) {
		// 		creature = summon;
        //     }
        // } // mixlort proteção contra attack player com summon
...
void Creature::addStorageStringValue(const uint32_t key, const std::string& value)
{
...
bool Creature::getStorageStringValue(const uint32_t key, std::string& value) const
{
