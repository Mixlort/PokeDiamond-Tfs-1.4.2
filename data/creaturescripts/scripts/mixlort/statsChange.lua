...
function onHealthChange(cid, attacker, value, combat)
    ...
    if isPlayer(cid) and #getCreatureSummons(cid) <= 0 and type == STATSCHANGE_HEALTHLOSS then
        ...
        if not isPlayer(cid) then
            addEvent(sendPlayerDmgMsg, 5, cid:getId(), "You lost "..valor.." hitpoints due to an attack from "..attacker:getName()..".")
        end
        return false
    end
    --------------------------------------------------
    ...
    if isSummon(attacker) then
        ...
        if isSummon(cid) and valor ~= 0 then
            addEvent(sendPlayerDmgMsg, 5, getCreatureMaster(cid:getId()):getId(), "Your "..getCreatureName(cid:getId()).." lost "..valor.." hitpoints due to an attack from "..attacker:getName()..".")
        end
    end
