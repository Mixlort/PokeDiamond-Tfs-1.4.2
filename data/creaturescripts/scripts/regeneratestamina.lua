function onLogin(player)
	...
	local maxNormalStaminaRegen = (staminaMinutesPadrao*60) - math.min((staminaMinutesPadrao*60), staminaMinutes)
	...
	if regainStaminaMinutes > maxNormalStaminaRegen then
		...
		staminaMinutes = math.min((staminaMinutesMaximo*60), math.max((staminaMinutesPadrao*60), staminaMinutes) + happyHourStaminaRegen)
	else
