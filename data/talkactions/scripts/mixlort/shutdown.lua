...
local function shutdownMessage(timeLeft)
    if timeLeft == 1 then
        broadcastMessage("O servidor ira desligar em 1 minuto. Por favor, deslogue!", MESSAGE_STATUS_WARNING)

        addEvent(shutdownServer, 1 * 60 * 1000)
        return
    end

    broadcastMessage("O servidor ira desligar em ".. timeLeft .." minutos!", MESSAGE_STATUS_WARNING)
