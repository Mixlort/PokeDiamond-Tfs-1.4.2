...
if getPlayerStorageValue(getNpcCid():getId(), 665461) <= 0 then
   setPlayerStorageValue(getNpcCid():getId(), 665461, math.random(1, 5))
...
if getPlayerStorageValue(getNpcCid():getId(), 154788) <= 0 then
   ...
   doChangeSpeed(getNpcCid():getId(), -getCreatureSpeed(getNpcCid():getId()))
   addEvent(randWalk, 2000, getNpcCid():getId(), 1000, getPlayerStorageValue(getNpcCid():getId(), 154788), target)
end
return true
end 
------------------------------------------------------
...
if fighting then
   ...
   if #getCreatureSummons(getNpcCid():getId()) == 0 and isCreature(target) then
      if battle_turn > #wildCommanderOfficer[getPlayerStorageValue(getNpcCid():getId(), 665461)] then      
