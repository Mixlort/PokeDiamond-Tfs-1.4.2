...
if getPlayerStorageValue(getNpcCid():getId(), 665471) <= 0 then
   setPlayerStorageValue(getNpcCid():getId(), 665471, math.random(1, 5))
...
if fighting then
   ...
   if #getCreatureSummons(getNpcCid():getId()) == 0 and isCreature(target) then
      if battle_turn > #wildHunter[getPlayerStorageValue(getNpcCid():getId(), 665471)] then      
