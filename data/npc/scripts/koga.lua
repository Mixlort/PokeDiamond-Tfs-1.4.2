...
function onCreatureSay(cid, type, msg)
	...
	if isDuelMsg(msg) and conv == 1 and focus == cid then
		...
		--if getPlayerItemCount(cid, gymbadges[getCreatureName(getNpcCid())]) >= 1 then
...
function onThink()
	...
	--doSendAnimatedText(getThingPos(getNpcCid()), getCreatureName(getNpcCid()), 215)
	...
	if fighting then
		...
		if #getCreatureSummons(getNpcCid()) == 0 then
			if battle_turn > #pokemons then
				addEvent(doWinDuel, 1000, focus, getNpcCid())
				setPlayerStorageValue(focus, 990, -1)
				focus = 0
			return true
			end
