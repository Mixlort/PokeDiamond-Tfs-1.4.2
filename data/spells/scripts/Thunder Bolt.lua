function onCastSpell(cid, var)
	...
	local function doThunderFall(cid, frompos, target)
		...
		addEvent(doDanoInTarget, ry * 11, cid:getId(), target:getId(), ELECTRICDAMAGE, min, max, 48) --alterado v1.7
	end
	...
	local function doThunderUp(cid, target)
		...
		addEvent(doThunderFall, rd * 49, cid:getId(), topos, target:getId())
	end		

    setPlayerStorageValue(cid, 3644587, 1)
	addEvent(setPlayerStorageValue, 350, cid:getId(), 3644587, -1)
    for thnds = 1, 2 do
		addEvent(doThunderUp, thnds * 155, cid:getId(), target:getId())
	end
