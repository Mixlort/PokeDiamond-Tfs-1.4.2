...
if isCreature(cid) then
   ...
   addEvent(doSendDistanceShoot, 350, posis, getThingPos(target:getId()), eff)
   addEvent(doSendDistanceShoot, 450, getThingPos(target:getId()), posis, eff)
   addEvent(doSendDistanceShoot, 600, posis, getThingPos(target:getId()), eff)
   addEvent(doSendDistanceShoot, 650, getThingPos(target:getId()), posis, eff)
   addEvent(nois, 670, cid:getId(), a)
   addEvent(doAppear, 670, cid:getId())
end
end
 ...
 eff = 20
   ...
   if #getCreatureSummons(master) >= 1 and isCreature(target) and not ehNPC(master) then
      if isCreature(cid) then
         ...
         addEvent(doTargetCombatHealth, 500, cid:getId(), target:getId(), BUGDAMAGE, -minn, -maxx, 255)
