...
if getPlayerStorageStringValue(cid, storage) == -1 or not string.find(getPlayerStorageStringValue(cid, storage), ";") then  
   setPlayerStorageStringValue(cid, storage, "normal = 0, great = 0, super = 0, ultra = 0, saffari = 0;")            
end     
...
if getPlayerStorageStringValue(cid, 98796) >= 1 and getPlayerItemCount(cid, 12617) <= 0 then  --alterado v1.9
   setPlayerStorageStringValue(cid, 98796, -1) 
   setPlayerStorageStringValue(cid, 98797, -1)                                              
   doTeleportThing(cid, SafariOut, false)
   doSendMagicEffect(getThingPos(cid), 21)
   doPlayerSendTextMessage(cid, 27, "You spend all your saffari balls, good luck in the next time...")
end

addEvent(doSendPokeBall, d * 70 + 100 - (d * 14) , cid:getId(), catchinfo, false, false, typeee) 
