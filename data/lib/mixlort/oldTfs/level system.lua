function getStatusW(name)
    ...
    local level = pokes[name] and pokes[name].wildLvl or 0
  	local status = {}
	local pokeAtk = pokes[name].offense
	status.atk = pokeAtk * level
    status.atk = status.atk / 100
	local pokeDef = pokes[name].defense
	status.def = pokeDef
	local pokeSpeed = pokes[name].agility
	status.speed = pokeSpeed
	local pokeVit = pokes[name].vitality
	status.vit = pokeVit * level
    local pokeLife = math.floor(status.vit)
    status.life = (pokeLife * HPperVITwild) / 100
	local pokeSpAtk = pokes[name].specialattack
	status.spAtk = pokeSpAtk * level
    local pokeSpDef = math.floor(status.spAtk) * 0.85 + math.floor(status.def) * 0.2
    status.spDef = pokeSpDef
	local pokeExp = pokes[name].exp
...
function getStatusS(name, boost, masterLevel, bonusdef, bonusboost)
    boost, bonusdef, bonusboost = boost or 0, bonusdef or 0, bonusboost or 0
    local newStats = MonsterType(name)
  	local status = {}
	local pokeAtk = pokes[name].offense
	status.atk = pokeAtk * (masterLevel + boost) + bonusboost
    status.atk = status.atk / 100
	local pokeDef = pokes[name].defense
	status.def = pokeDef * bonusdef + bonusboost
	local pokeSpeed = pokes[name].agility
	status.speed = pokeSpeed
	local pokeVit = pokes[name].vitality
	status.vit = pokeVit * (masterLevel + boost) + bonusboost
    local pokeLife = math.floor(status.vit)
    status.life = (pokeLife * HPperVITsummon) / 100
	local pokeSpAtk = pokes[name].specialattack
	status.spAtk = pokeSpAtk * (masterLevel + boost) + bonusboost
    local pokeSpDef = math.floor(status.spAtk) * 0.85 + math.floor(status.def) * 0.2
    status.spDef = pokeSpDef
	local pokeExp = pokes[name].exp
	status.exp = pokeExp * baseExpRate + pokeVit * pokemonExpPerLevelRate
...
function getStatusSummon(poke)
	...
	if not poke then return false end
	local name = poke:getName()
	local boost, heldDef, heldBoost = 0, 0, 0
    if poke:getMaster() then
        ...
        if master:getUsingBall() then
            ...
            heldDef = getBonusHeldDef(ball.uid) or 0
            heldBoost = getBonusHeldBoost(ball.uid) or 0
        end
    end
	local masterLevel = poke:getMaster():getLevel()
    local status = {}
    if not getStatusS(name, boost, masterLevel, heldDef, heldBoost) then return 1 end
    status.atk = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).atk) or 1
    status.def = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).def) or 1
    status.speed = getStatusS(name, boost, masterLevel, heldDef, heldBoost).speed or 1
    status.vit = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).vit) or 1
    status.life = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).life) or 1
    status.spAtk = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).spAtk) or 1
    status.spDef = math.floor(getStatusS(name, boost, masterLevel, heldDef, heldBoost).spDef) or 1
    status.exp = getStatusS(name, boost, masterLevel, heldDef, heldBoost).exp or 1
    return status
end
...
function getBonusHeldBoost(item)
    -- Defense -- 
    local bonusboost = 0
    local Tiers = {
        [22] = {bonus = BoostBonus1},
        [23] = {bonus = BoostBonus2},
        [24] = {bonus = BoostBonus3},
        [25] = {bonus = BoostBonus4},
        [26] = {bonus = BoostBonus5},
        [27] = {bonus = BoostBonus6},
        [28] = {bonus = BoostBonus7},
    }
    local Tier = getItemAttribute(item, "heldx")
    if Tier and Tier > 21 and Tier < 29 then
        bonusboost = Tiers[Tier].bonus
    end
    return bonusboost
    -- Defense --
end
