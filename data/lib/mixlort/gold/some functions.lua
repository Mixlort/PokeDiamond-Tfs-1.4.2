...
if not pokes[pokemon] then return false end
   ...
   if (getPlayerFreeCap(cid) <= 1 and not isInArray({3, 2}, getPlayerGroupId(cid))) or not hasSpaceInContainer(getPlayerSlotItem(cid, 3).uid) then 
      item = depot:addItem(11826, 1, INDEX_WHEREEVER, FLAG_NOLIMIT)
   else
      item = cid:addItem(11826, 1)   
   end
...
function getLivePokeballs(cid, container, duel) 
	...
	local items = {}
	---
	local ballSlot = cid:getUsingBall()
   if not ballSlot then return true end
...
function getPokemonHappinessDescription(cid)
	...
	if h >= PokemonStageVeryHappy then
		table.insert(str, " is very happy with you!")
	elseif h >= PokemonStageHappy then
		table.insert(str, " is happy.")
	elseif h >= PokemonStageOK then
		table.insert(str, " is unhappy.")
	elseif h >= PokemonStageSad then
		table.insert(str, " is sad.")
	elseif h >= PokemonStageMad then
...
function doReturnPokemon(cid, pokemon, pokeball, effect, hideeffects, blockevo)
	...
	if not pokeball then
		pokeball = cid:getUsingBall()
      if not pokeball then return true end
	end
...
--    print("ERRO, usar getCreatureLookDirection") --mixlort
   ...
   return getCreatureLookDirection(cid) --mixlort
	-- local dirs = {
	-- [NORTHEAST] = {NORTH, EAST},
	-- [SOUTHEAST] = {SOUTH, EAST},
	-- [NORTHWEST] = {NORTH, WEST},
...
--    if Creature(sid) and getCreatureMaster(sid) ~= Creature(sid) and isPlayer(getCreatureMaster(sid)) then
--       return false 
--    else
--       return true
--    end
