...
function doDittoTransform(ditto, pokemon)
    ...
    if (getCreatureName(ditto) == "Ditto") then
        ...
        doItemSetAttribute(getPlayerSlotItem(sid, 8).uid, "pokeName", name)
...
function doDittoRevert(m)
    ...
    if getItemAttribute(ball, "ehditto") then
        if #getCreatureSummons(sid) <= 0 then
            doItemSetAttribute(ball, "pokeName",  "Ditto")
    ...
    elseif getItemAttribute(ball, "ehshinyditto") then
        if #getCreatureSummons(sid) <= 0 then
            doItemSetAttribute(ball, "pokeName", "Shiny Ditto")
...
--   -- if pokeballs[pokemon:lower()] then
...
--   doItemSetAttribute(item, "pokeName", pokemon)  
...
--   doItemSetAttribute(item, "pokeHealth", 1)
...
--   if not isGod(cid) and getPlayerFreeCap(cid) >= 6 or not hasSpaceInContainer(getPlayerSlotItem(cid, 3).uid) then  
...
function getPlayerDesc(cid, thing)
if (not isCreature(cid) or not isCreature(thing)) then return "" end

local pos = getThingPos(thing)
local ocup = youAre[getPlayerGroupId(thing)]
local rank = (getPlayerStorageValue(thing, 86228) <= 0) and "um Treinador Pokemon" or lookClans[getPlayerStorageValue(thing, 86228)][getPlayerStorageValue(thing, 862281)]
...
local str = {}
table.insert(str, "Você vê "..name.." ")
table.insert(str, "(Nível "..getPlayerLevel(thing)..")\n")
table.insert(str, art.." ")
...
if getPlayerGuildId(thing) > 0 then
   table.insert(str, " "..art.." "..getPlayerGuildRank(thing).." de "..getPlayerGuildName(thing).."")
end
table.insert(str, ((isPlayer(cid) and youAre[getPlayerGroupId(cid)]) and "Position: (X: "..pos.x..", Y: "..pos.y..", Z: "..pos.z..")" or "")) 
if isGod(cid) then
...
--               if duel and getPlayerLevel(cid) >= (pokes[getItemAttribute(ballSlot.uid, "pokeName")].level + getPokeballBoost(ballSlot)) then
...
--                            if duel and getPlayerLevel(cid) >= (pokes[getItemAttribute(item.uid, "pokeName")].level + getPokeballBoost(item)) then    
...
addEvent(removerPlayer, 5*1000, cid:getId())     
return TRUE
end
...
-- function doCorrectString(str) -- mixlort 2.0 statussyst
...
function doReturnPokemon(cid, pokemon, pokeball, effect, hideeffects, blockevo)
	...
	if edit then
		...
		doItemSetAttribute(pokeball.uid, "pokeHealth", pokelife)
	end
    ...
    if isMegaAttr and megaEvolutions[isMegaAttr] then
		...
		if backMega then
		    doItemSetAttribute(pokeball.uid, "pokeName", backMega[1])
		end
	end
    ...
    doSendDistanceShoot(fromPosition, toPosition, balls["pokeName"].projectile)

    -- addEvent(doSendProjectile, 350, toPosition, cid, balls["pokeName"].projectile)

    addEvent(function()
        if isCreature(cid:getId()) then
            doSendProjectile(toPosition, cid:getId(), balls["pokeName"].projectile)
        end
    end, 350)
...
-----------------------------------------------------------------
	...
	local thishp = getItemAttribute(item.uid, "pokeHealth")
...
if not isInArray({5, 6}, getPlayerGroupId(cid)) then
   ...
   if #balls >= 1 then
      for _, uid in ipairs(balls) do
          local nome = getItemAttribute(uid, "pokeName")
...
---------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if getPlayerLevel(cid) < (monsterType:getLevelmax() + boosts) then   --alterado v1.8 \/
	doPlayerSendCancel(cid, "You need level "..(monsterType:getLevelmax() + boosts).." to use this pokemon.")
	return true
	end
    ...
    local pokemonName = getItemAttribute(ball.uid, "pokeName")
...
function doRegainSpeed(cid) --mixlort remover
	...
	elseif isPlayer(cid) and getPlayerStorageValue(cid, 5700) > 0 then
	    speed = speed + getPlayerLevel(cid) + getPlayerStorageValue(cid, 5700)
	elseif isPlayer(cid) then
	    speed = speed + getPlayerLevel(cid)
    end
...
function getResistance(cid, combat)
    ...
    local monsterType = MonsterType(cid:getName())
local poketype1 = monsterType:getRaceName()
local poketype2 = monsterType:getRace2Name()
local multiplier = 1
...
function getFreeTile(pos, cid)
	...
	local positions = {}
	for a = 0, 7 do
		if canWalkOnPos(getPositionByDirection(pos, a), true, false, true, true, false) then
...
function doTruant(cid)
	...
	addEvent(doTruant, 15000, cid:getId())
end
...
function getPokemonPrice(item)
	-- if not pokes[getItemAttribute(item, "pokeName")] then return 0 end
stoneprice = 0
price = 0
name = getItemAttribute(item, "pokeName")
boost = getItemAttribute(item, "boost") or 0
	if boost >= 1 then
        local monsterType = MonsterType(name)
		stoneid = doConvertTypeToId(monsterType:getRaceName())
		stoneprice = getStonePrice(stoneid) * boost
	end
	price = pokePrice[name] or monsterType:getMaxlevel() * 150
...
stoneprice = 0
	...
	if #balls >= 1 then
		for i = 1, #balls do
			-- if pokes[getItemAttribute(balls[i], "pokeName")] then
				name = getItemAttribute(balls[i], "pokeName")
				boost = getItemAttribute(balls[i], "boost") or 0
                local monsterType = MonsterType(getItemAttribute(balls[i], "pokeName"))
				if boost >= 1 then
					stoneid = doConvertTypeToId(monsterType:getRaceName())
					stoneprice = getStonePrice(stoneid) * boost
				end
...
balls = getPokeballsInContainer(container)
	...
	if #balls >= 1 then
		for i = 1, #balls do
			local name = getItemAttribute(balls[i], "pokeName")
                ...
                if Tier and Tier > 0 and Tier < 8 then
                    if not getCreatureCondition(cid, CONDITION_INFIGHT) then
                        if getCreatureHealth(sid) < getCreatureMaxHealth(sid) then
                            addEvent(function() 
                                if isCreature(sid:getId()) then
                                    doCreatureAddHealth(sid:getId(), Tiers[Tier].bonus) 
                                    doSendAnimatedText(getThingPos(sid:getId()), "+ "..Tiers[Tier].bonus, 30) 
                                end
                            end, 0)
                        end
...
name = getItemAttribute(pokeballs[i], "pokeName")
...
local shiny = getItemAttribute(pokeball, "shiny")
return shiny and "Shiny "..getItemAttribute(pokeball, "pokeName").."" or getItemAttribute(pokeball, "pokeName")
end
 
 
function setStatusPokeballByStatusStr(pokeball, status)
local attr = string.explode(status, ",")
local x = {"pokeName", "pokeHealth", "offense", "defense", "speed", "vitality", "specialattack", "happy", "gender", "hands", "description", "fakedesc", "boost", "ball", "defeated", "shiny"}
for i =1, #attr do
...
if not getItemAttribute(item.uid, "pokeName") then
return true
end    
for i, x in pairs(fotos) do
if string.lower(getItemAttribute(item.uid, "pokeName")) == string.lower(i) then
...
t = getPlayerSlotItem(cid, 8)
if t.uid ~= 0 then
pokeslot = getItemAttribute(t.uid, "pokeName")
...
local pokeslot = getItemAttribute(poke.uid, "pokeName")
...
local pokeballs = getPokeballsInContainer(container)
for i =1, #pokeballs do
pokemons = getItemAttribute(pokeballs[i], "pokeName")
...
--   for i =1, #balls do
--     local ball = balls[i]
--     -- print("Pokemon: "..pokemon.." attrpoke> "..(getItemAttribute(ball.uid, "pokeName") or "sempoke").." attrnick> "..(getItemAttribute(ball.uid, "nick") or "sem nick"))
--     if getItemAttribute(ball.uid, "pokeName") == pokemon or getItemAttribute(ball.uid, "nick") == pokemon then
--       -- print("IGUAL")
...
--           doSetItemAttribute(ball.uid, "pokeHealth", 1)
--           local name = getItemAttribute(ball.uid, "pokeName")
--           for c = 1, 15 do
--             local str = "move"..c
...
if not ball or ball <= 1 then return true end
    local monsterType = MonsterType(getItemAttribute(ball, 'poke'))
    return monsterType:getMaxHealth() * (getPlayerLevel(cid) + (getItemAttribute(ball, 'boost') or 0))
end
...
function onPokeHealthChange(cid, zerar)
   ...
   if #getCreatureSummons(cid) >= 1 and getPlayerStorageValue(cid, 212124) <= 0 then   --alterado v1.6
      local pokemon = getCreatureSummons(cid)[1]
      local pokelife = (getCreatureHealth(pokemon) / getCreatureMaxHealth(pokemon))
      doItemSetAttribute(ball, "pokeHealth", pokelife)
   end
   
      local rd = 1 - (tonumber(getItemAttribute(ball, "pokeHealth")))
