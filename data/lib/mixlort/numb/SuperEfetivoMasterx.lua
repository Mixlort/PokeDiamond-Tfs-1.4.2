function sendDittoWindow(cid, item)
	local pokename = tostring(getItemAttribute(item.uid, "pokeName"))
...
--     local chancePassiva = math.random(1, 100)
--     local randomPassive = getPassiveRandomSpell(cid)
--     if randomPassive then
--         if chancePassiva <= 1 then
--             -- doCreatureSay(cid, randomPassive, TALKTYPE_MONSTER)
--             doCreatureCastSpell(cid, randomPassive)
--         end
--     end
...
function getPokemonAdvantage(pokename)
	pokename = retireShinyName(doCorrectString(pokename))
	local stats = {}
...
-- 		local poke = getItemAttribute(ball.uid, "pokeName")
-- 		local boostHeld = 0
...
function doSurf(cid)
	...
	local pokeName = getItemAttribute(getPlayerSlotItem(cid, 8).uid, "pokeName")
	...
	local speed = 75 + PlayerSpeed + getStatus(getCreatureSummons(cid)[1]).speed * 8
...
function doGoPokemon(cid, ball, sendEffect)
	if isPlayer(cid) then
		...
		if isPokeball(ball.itemid) then
			...
			if getItemAttribute(ball.uid, "pokeHealth") and getItemAttribute(ball.uid, "pokeHealth") <= 0 then
				doTransformItem(ball.uid, pokeballs[btype].off)
				doItemSetAttribute(ball.uid, "pokeHealth", 0)
				doPlayerSendCancel(cid, "This pokemon is fainted.")			
				return true
			end

			local pokemon = getItemAttribute(ball.uid, "pokeName")
			
			-- if not pokes[pokemon] then
			-- 	return true
			-- end
			
			local btype = getPokeballType(ball.itemid) or "normal"
			...
			if not isInArray({"Charmander", "Squirtle", "Bulbasaur"}, pokemon) then
				if getPlayerLevel(cid) < (monsterType:getMaxlevel()+boost) then
					doPlayerSendCancel(cid, "You need level "..(monsterType:getMaxlevel()+boost).." to use this pokemon.")
					return true
				end
			end
			
			local nick = getItemAttribute(ball.uid, "pokeName")
			...
			if getItemAttribute(ball.uid, "copyName") then -- ditto system
				...
				pokemonRealName = getItemAttribute(ball.uid, "pokeName")
			end
