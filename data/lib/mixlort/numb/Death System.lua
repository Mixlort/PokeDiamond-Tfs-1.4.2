...
local function playerAddExp(cid, exp)
	...
	-- print("testemixlortxp1") -- MIXLORT REVER
...
local playerExperienceRate = 1 -- MIXLORT ISSO INFLUENCIA XP
...
-- local function doCorpseAddLoot(name, item, cid, target, corpse2)
...
-- 					local corpse = MonsterType(getCreatureName(target)):getCorpseId()
...
--         local expTotal = 0
--         if givenexp and givenexp > 0 then
--             local pk = target
...
-- 		doItemSetAttribute(corpse2, "pokeName", getCreatureName(target))
...
function doKillPlayer(cid, attacker, hit)
	...
	if isPremium(cid) then
		exp = exp * 0.75
		doPlayerSendTextMessage(cid, 20, "Você perdeu apenas 75% da experiência que perderia se não fosse VIP.")
	end
...
function doKillPlayerPokemon(cid)
	...
	local ballName = getItemAttribute(thisball.uid, "pokeName")
	...
	if isInArray({"Castform Water", "Castform Ice", "Castform Fire"}, ballName) then
		doSetItemAttribute(thisball.uid, "pokeName", "Castform")
		return true
	end
	...
	doItemSetAttribute(thisball.uid, "pokeHealth", 0)
	...
	local Pokeboss = {
		...
		if corpseID ~= 0 then
			...
			if corpse and not isSummon(target) then
				if nameDeath then
					local success, error = pcall(doItemSetAttribute, corpse, "pokeName", "fainted " .. nameDeath:lower())
					if success then
						doItemSetAttribute(corpse, "pokeName", "fainted " .. nameDeath:lower())
						-- doItemSetAttribute(corpse, "level", target:getLevel())
						...
						print("Erro ao adicionar atributo ao corpse:", error)
					end
				end
			end
		end
