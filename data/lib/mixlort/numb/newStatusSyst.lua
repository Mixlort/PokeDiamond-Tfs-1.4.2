...
function doSendSleepEffect(cid)
    ...
    doSendMagicEffect(cid:getPosition(), 32)
...
function doDamageInTargetWithDelay(cid, target, dMin, dMax, effect, dType) -- refeita 
    ...
    if isNumber(target) then target = Creature(target) end
    if not isCreature(cid) or not isCreature(target) then return true end 
    local delay = getDistanceBetween(cid:getPosition(), getThingPos(target)) * const_distance_delay
    addEvent(doDamageInTarget, delay, cid:getId(), target:getId(), dMin, dMax, effect, dType)
end
...
function doCondition2(ret)
    ...
    function doMiss2(cid, cd, eff, check, spell)
        ...
        doSendMagicEffect(cid:getPosition(), eff)
    ...
    function doConfusion2(cid, cd, check)
        ...
        if math.random(1, 6) >= 4 then
            doSendMagicEffect(cid:getPosition(), 31)
        end
    ...
    function doPoison2(cid, cd, check, damage)
        ...
        if not isCreature(cid) then return true end --is creature?
        ----------
        if isSummon(cid) or ehMonstro(cid) then --alterado v1.6
    ...
    function doFear2(cid, cd, check, skill)
        ...
        addEvent(doFear2, 400, cid:getId(), -1, a, skill) 
    end 
        ...
        if first and not isPlayer(cid) then 
            ...
            elseif not isPlayer(cid) and not isMega(cid) then
                if isInArray({604, 605, 1015, 1016, 1183, 1184}, getCreatureOutfit(cid).lookType) then
                    ...
                    Info = MonsterType(getCreatureName(cid)):getCorpseId()
                end 
                local look = getCreatureOutfit(cid) 
                ---------
                local name = cid:getName()
                -- if name == "Ditto" and isSummon(cid) and name and name ~= "Ditto" then
                    local InfoDitto = MonsterType(name):getCorpseId()
                    if InfoDitto ~= 0 and look.lookType ~= 0 then 
                        doSetCreatureOutfit(cid, {lookType = 0, lookTypeEx = MonsterType(name):getCorpseId()}, -1)
                    end 
                -- else
    ...
    function doLeech2(cid, attacker, cd, check, damage)
        ...
        local life = getCreatureHealth(cid) or 0
        
        -- if not damage then print("notdamage") damage = 0 return true end
        if not damage then damage = 0 end
        if damage < 0 or damage == -0 then 
            damage = damage * -1
        end

        if damage >= life then
            doSendAnimatedText(cid:getPosition(), "-"..damage.."", 144)
            doSendAnimatedText(attacker:getPosition(), "+"..damage.."", 32)
        ...
        doCreatureAddHealth(cid, -damage)
        doSendAnimatedText(cid:getPosition(), "-"..damage.."", 144)
        doSendMagicEffect(cid:getPosition(), 45)
        ------
        local newlife = life - getCreatureHealth(cid)
        if newlife >= 1 and attacker ~= 0 then
            doSendMagicEffect(attacker:getPosition(), 14)
            doCreatureAddHealth(attacker, newlife)
            doSendAnimatedText(attacker:getPosition(), "+"..newlife.."", 32)
            local dano = getCreatureHealth(cid)-damage <= 0 and getCreatureHealth(cid)-1 or damage 
    ...
    function doBuff2(cid, cd, eff, check, buff, first, attr)
        ...
        if buff == "Protect" then
            local posC2 = getThingPosWithDebug(cid:getId()) posC2.x = posC2.x+1 posC2.y = posC2.y+1
            doSendMagicEffect(posC2, 486)
        else
            doSendMagicEffect(cid:getPosition(), eff) -- posição default para o ret.effect
        end
        
        if buff == "Magnetic Flux" then
            local posC1 = getThingPosWithDebug(cid:getId()) posC1.x = posC1.x+1 posC1.y = posC1.y+1
        ...
        if first then
            ...
            elseif buff == "Rage" then
                ...
                addEvent(setPlayerStorageValue, a * 1000, cid:getId(), 463523, -1)
...
function doMoveInArea2(cid, eff, area, element, min, max, spell, ret)
    ...
    if pos == "true" then
        print("Erro de area na spell: "..(spell or ret or "ERRO1"))
        return true
    end
        ...
        if pid ~= cid then
            if spell and isInArray(skills, spell) then
                if spell and spell == "Gyro Ball" then --alterado v1.7
                    ...
                    addEvent(sendEffWithProtect, l*200, cid:getId(), pos[n], eff)
                    addEvent(doMoveDano2, l*200, cid:getId(), pid, element, min, max, ret, spell) --alterado v1.6 
                end
            elseif spell and spell == "Epicenter" then
                ...
                addEvent(doDanoWithProtect, random, cid:getId(), GROUNDDAMAGE, pos[n], crusher, -min, -max, 255)
            ...
            elseif spell and spell == "Surf" then
                ...
                addEvent(doMoveDano2, 400, cid:getId(), pid, element, min, max, ret, spell) --alterado v1.6 
            elseif spell and spell == "Sand Attack" then
                addEvent(sendEffWithProtect, n*200, cid:getId(), pos[n], eff)
                addEvent(doMoveDano2, n*200, cid:getId(), pid, element, min, max, ret, spell) --alterado v1.6 
            ...
            elseif spell == "Last Resort" then
                ...
                local pos = getThingPosWithDebug(cid:getId())
                local areas = {rock5, rock4, rock3, rock2, rock1, rock5, rock4, rock3, rock2, rock1}
                
                for i = 0, 9 do
                    addEvent(doMoveInArea2, i*400, cid:getId(), 3, areas[i+1], NORMALDAMAGE, min, max, spell)
                    addEvent(doMoveInArea2, i*410, cid:getId(), 3, areas[i+1], NORMALDAMAGE, 0, 0, spell)
                end
            else
...
function doMoveDano2(cid, pid, element, min, max, ret, spell) -- add para não receber dano em área se o atacado for boss or shiny.
    ...
    if isCreature(pid) and isCreature(cid) and cid ~= pid then
        ...
        if getPlayerStorageValue(pid, 21099) >= 1 then
            doSendMagicEffect(getThingPosWithDebug(pid:getId()), 135)
            doSendAnimatedText(getThingPosWithDebug(pid:getId()), "REFLECT", COLOR_GRASS)
...
function upEffect(cid, effDis)
    ...
    doSendDistanceShoot(cid:getPosition(), frompos, effDis)
end
---------------------------------------------------------------------------------
function fall(cid, master, element, effDis, effArea) --Function pra jogar efeitos pra cima e cair depois... tpw falling rocks e blizzard
    ...
    if isCreature(cid) then
        ...
        -- if effArea and effArea > 0 then effArea = effArea + 1 end
...
function canDoMiss(cid, nameAtk) --alterado v1.5
    ...
    if isInArray(atkTerra, nameAtk) then
        if (monsterType:getRaceName() == "flying") or (monsterType:getRace2Name() == "flying") or isInArray(specialabilities["Levitate"], getCreatureName(cid)) then
            return false 
        end
        ...
        if getPlayerStorageValue(cid, 374896) >= 1 then
            if getMasterTarget(cid) == pid then
                if isInArray({"Kadabra", "Alakazam", "Mew", "Shiny Abra", "Shiny Alakazam"}, getCreatureName(cid)) then
                    doSendDistanceShoot(getThingPosWithDebug(cid:getId()), getThingPosWithDebug(pid:getId()), 39)
                end
                if isSummon(cid) then
                    doTargetCombatHealth(getCreatureMaster(cid), pid, PHYSICALDAMAGE, -math.abs(valor), -math.abs(valor), 255)
                else
                    doCreatureAddHealth(pid, -math.abs(valor), 3, races[MonsterType(getCreatureName(pid)):getRaceName()].cor)
                end
            end
        end
    end
